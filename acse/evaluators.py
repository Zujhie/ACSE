...
def extract_features(model, data_loader, print_freq=50):
    ...
    features = OrderedDict()    # 用于存储提取的特征
    labels = OrderedDict()      # 用于存储对应的标签

    end = time.time()
    with torch.no_grad():       # 禁用梯度计算，在评估模式下不需要反向传播
...
def pairwise_distance(features, query=None, gallery=None):
    ...
    dist_m.addmm_(1, -2, x, y.t())      # dist_m = dist_m - 2 * torch.mm(x, y.t())
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
    ...
    # cmc_configs为字典，用于设置CMC计算的参数   只提供 market1501的配置【其他？？】
    cmc_configs = {
